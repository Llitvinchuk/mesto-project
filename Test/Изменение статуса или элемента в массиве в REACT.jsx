...
class CalendarEvent extends React.Component {
  ...
  getCurrentUserConfirmationStatus() {
    ...
    const user = invited.find(item => currentUser === item.id);
    return user.status;
  }
  ...
  confirm = () => {
    const currentUser = this.state.currentUser;
    const invited = this.state.invited;
    const newInvited = invited.map(item =>{
      if(currentUser === item.id){
        return{...item, status: STATUS_CONFIRMED}
      }
      return item;
    });
  ...
  cancel = () => {
    ...
    const newInvited = invited.map(item =>{
      if(currentUser === item.id){
        return{...item, status: STATUS_CANCELED}
      }
      return item;
    });
    this.setState(prevState =>{
