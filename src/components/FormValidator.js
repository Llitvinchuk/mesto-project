export default class FormValidator{
	...
	_setEventListener(formElement){
		const contentSelector = Array.from(formElement.querySelectorAll(this._options.contentSelector));
	  const buttonSave = formElement.querySelector(this._options.submitButtonSelector);
	...
	_showError(input, form, errorMessage){
		const errorSpan = form.querySelector(`.${input.id}-` + this._options.titleError);  
